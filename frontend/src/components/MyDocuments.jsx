...
function AddModal({ userId, onClose, onSaved, editItem }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (mode === "file" && file) {
        // Get current session for upload
          ...
          <div>
            ...
            <div className="relative">
              <select value={category} onChange={(e) => setCategory(e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-teal-400 appearance-none bg-white pr-8 transition">
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 hover:border-teal-200 hover:shadow-sm transition group">
      <div className="flex items-start gap-3">
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${isNote ? "bg-yellow-50" : "bg-blue-50"}`}>
          {isNote
        ...
        {/* Actions */}
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 md:opacity-0 md:group-hover:opacity-100 transition">
          {!isNote && (
            <>
...
export default function MyDocuments({ userId }) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      {/* Header */}
      <div className="bg-white border-b border-gray-100 px-4 md:px-5 py-4 flex-shrink-0">
        ...
        <div className="relative mb-3">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            ...
            className="w-full pl-8 pr-3 py-2 rounded-xl border border-gray-200 text-xs focus:outline-none focus:border-teal-400 transition" />
          {search && (
      ...
      {error && (
        <div className="mx-4 mt-3 p-3 rounded-xl bg-red-50 border border-red-100 flex gap-2 flex-shrink-0">
          <AlertCircle size={14} className="text-red-500 flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-xs text-red-600 font-medium">Database error</p>
