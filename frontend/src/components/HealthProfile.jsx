...
import { User, Plus, X, Save, Loader2, CheckCircle, AlertCircle } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-3 min-h-[34px]">
        {values.length === 0 && (
          <span className="text-xs text-gray-400 py-1 italic">None added yet</span>
        )}
        {values.map((v) => (
          <span key={v}
            className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-teal-50 text-teal-700 border border-teal-200 font-medium">
            {v}
      ...
      <div className="flex gap-2 mb-3">
        <input value={val}
          ...
          className="flex-1 px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-50 transition"
        />
        <button onClick={() => add(val)}
          className="px-3 py-2 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-teal-300 transition">
          <Plus size={15} />
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {suggestions.filter((s) => !values.includes(s)).map((s) => (
          <button key={s} onClick={() => add(s)}
            className="text-xs px-2.5 py-1 rounded-full bg-gray-50 border border-gray-200 text-gray-500 hover:border-teal-300 hover:text-teal-700 hover:bg-teal-50 transition">
            + {s}
          </button>
        ))}
      </div>
    ...
    <div className="h-full overflow-y-auto bg-gray-50 p-4 md:p-6">
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Saved permanently · Personalizes AI responses, news & recommendations
            </p>
          </div>
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-amber-50 rounded-xl p-3">
                ...
                <p className="text-sm text-amber-800 font-semibold capitalize mt-0.5">
                  {profile.sugar_trend || "Unknown"}
                </p>
              </div>
              <div className="bg-red-50 rounded-xl p-3">
                <p className="text-xs text-red-500 font-medium">BP Trend</p>
                <p className="text-sm text-red-700 font-semibold capitalize mt-0.5">
        ...
        <p className="text-xs text-gray-400 text-center pb-4">
          Data stored securely in Supabase · Never lost on refresh
        </p>
      </div>
    </div>
  );
